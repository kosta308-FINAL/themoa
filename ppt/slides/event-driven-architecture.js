...
#s-evtarch .col{ flex:1; background:#fff; border-radius:20px; box-shadow:0 20px 46px rgba(0,0,0,.14);
  padding:26px 30px; }
#s-evtarch .col .chead{ display:flex; align-items:center; gap:10px; margin-bottom:20px; }
...
#s-evtarch .sitem2{ display:flex; align-items:flex-start; gap:16px; margin-bottom:20px; }
#s-evtarch .sitem2:last-child{ margin-bottom:0; }
...
#s-evtarch .sitem2 .stitle2{ color:#171717; font-size:17px; font-weight:800; margin-bottom:6px; }
#s-evtarch .sitem2 .sdesc2{ color:#555; font-size:15px; font-weight:500; line-height:1.6; }
#s-evtarch .sitem2 .scode{ display:inline-block; margin-top:6px; background:#F3F5F7; color:#333;
  font-size:13px; font-weight:700; padding:4px 10px; border-radius:8px; }

#s-evtarch .note{ position:absolute; left:70px; top:930px; width:1780px; background:#fff; border-radius:14px;
    ...
    <div class="col codef">
      ...
      <div class="sitem2">
        ...
        <div>
          ...
          <div class="sdesc2">카드사 로그인 정보는 RSA로 암호화해 EasyCodef SDK에 전달만 하고,<br>발급받은 connectedId만 영속화합니다. 로그인 자격증명은 저장하지 않습니다.</div>
        </div>
      </div>
      <div class="sitem2">
        ...
        <div>
          ...
          <div class="sdesc2">연동 성공 시 CardConnectionEstablishedEvent를 발행하고,<br>별도 리스너가 트랜잭션 커밋 이후 승인내역 수집을 비동기로 실행합니다.</div>
      ...
      <div class="sitem2">
        ...
        <div>
          ...
          <div class="sdesc2">CF-12801(비밀번호 오류) 등 CODEF 실패코드별로 분기 처리하고,<br>반복 실패 시 계정을 잠근 뒤 REQUIRES_NEW 트랜잭션으로 잠금 상태만 먼저 커밋합니다.</div>
        </div>
      </div>
    </div>
    ...
    <div class="col evt">
      ...
      <div class="sitem2">
        ...
        <div>
          ...
          <div class="sdesc2">SavingsSubscriptionService는 가입·해지 시 이벤트만 발행할 뿐,<br>FixedExpense 도메인의 존재 자체를 참조하지 않습니다.</div>
        </div>
      </div>
      <div class="sitem2">
        <div class="snum2">02</div>
        <div>
          ...
          <div class="sdesc2">FixedExpense 쪽 리스너가 이벤트를 구독해 카테고리 SAVING으로<br>자동 등록하며, 중복 저장 여부를 먼저 조회해 재실행에도 안전합니다.</div>
          <div class="scode">@Async @TransactionalEventListener(AFTER_COMMIT)</div>
        </div>
      </div>
      <div class="sitem2">
        ...
        <div>
          ...
          <div class="sdesc2">fixedexpense 패키지는 이벤트 클래스만 import하고,<br>subscription 패키지에는 fixedexpense를 참조하는 코드가 0건입니다.</div>
        </div>
      </div>
    </div>
  </div>
