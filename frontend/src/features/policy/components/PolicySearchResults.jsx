    ...
    <div className="policy-results">
      ...
      {result && totalPages > 0 && (
        <div className="policy-pagination">
          <button type="button" disabled={!hasPreviousPage || loading} onClick={() => onPageChange(page - 1)}>
            이전
          </button>
          <span>{page + 1}</span>
          <button type="button" disabled={!hasNextPage || loading} onClick={() => onPageChange(page + 1)}>
            다음
          </button>
        </div>
      )}
