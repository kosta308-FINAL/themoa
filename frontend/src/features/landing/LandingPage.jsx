    ...
    <section className="landing-hero" id="landing-hero">
      ...
      <div className="landing-shell landing-hero-layout">
        <div className="landing-hero-copy">
          <p className="landing-hero-badge">내 월급부터 다음 월급날까지</p>
          <h1 className="landing-hero-title">
            ...
            <span className="landing-hero-title-line">계획 있게.</span>
          </h1>
...
function HeroMiniDashboard() {
  return (
    <aside className="landing-mini-dashboard" aria-label="서비스 화면 예시">
      <div className="landing-preview-label">서비스 화면 예시</div>
      <div className="landing-mini-head">
        <span className="landing-mini-title">오늘 사용 가능 금액</span>
        <strong className="landing-mini-amount">110,000원</strong>
      </div>
      <div className="landing-mini-progress" aria-hidden="true">
        <span className="landing-mini-progress-fill" />
      </div>
      <div className="landing-mini-grid">
        <PreviewMetric label="이번 주기 남은 예산" value="1,540,000원" />
        <PreviewMetric label="관심 정책" value="3개" />
        <PreviewMetric label="관심 금융상품" value="1개" />
      </div>
      <div className="landing-mini-list">
        <p className="landing-mini-list-title">최근 소비 예시</p>
        <PreviewRow label="카페" value="-6,500원" />
        <PreviewRow label="정기 구독" value="-29,000원" />
      </div>
    </aside>
  );
}
      ...
      <div className="landing-shell">
        ...
        <div className="landing-problem-grid">
          {problemCards.map((text) => (
            <article className="landing-problem-card" key={text}>
              <SimpleIcon />
...
function FeatureBlock({ title, desc, preview, reverse = false }) {
  return (
    ...
    >
      ...
      <div className="landing-feature-preview">{preview}</div>
    </article>
  );
}
    ...
    <div className="landing-ui-card">
      ...
      <div className="landing-split-preview">
        <div className="landing-list-panel">
          ...
          <PreviewRow label="통신비 · 매월 12일" value="64,000원" />
          <PreviewRow label="정기 구독 · 매월 18일" value="29,000원" />
        </div>
        <div className="landing-list-panel">
          <p className="landing-panel-title">카테고리 소비 비중</p>
          <CssBars />
          <PreviewRow label="최근 거래" value="카페 -6,500원" />
        </div>
      </div>
    </div>
  );
...
function SearchPreview() {
  return (
    <div className="landing-ui-card">
      ...
      <div className="landing-result-card landing-result-card-policy">
...
function BookmarkPreview() {
  return (
    <div className="landing-ui-card">
      <div className="landing-preview-label">서비스 화면 예시</div>
      <div className="landing-bookmark-tabs">
        <span className="landing-bookmark-tab-active">마이페이지 북마크</span>
        <span>관심 정책 목록</span>
        <span>관심 상품 목록</span>
      </div>
      <PreviewRow label="청년 월세 지원" value="정책" />
      <PreviewRow label="청년 우대 적금" value="금융상품" />
    </div>
  );
}
    ...
    >
      <div className="landing-shell">
        ...
        <ol className="landing-step-list">
          {steps.map((step) => (
            <li className="landing-step-item" key={step.number}>
    ...
    <section className="landing-section landing-preview" id="landing-preview">
      <div className="landing-shell">
        ...
        <div className="landing-browser-frame">
          <div className="landing-browser-top" aria-hidden="true">
            <span />
            <span />
            <span />
          </div>
          <div className="landing-browser-content">
            ...
            <div className="landing-dashboard-grid">
              <PreviewMetric label="오늘 사용 가능 금액" value="110,000원" />
              <PreviewMetric label="이번 주기 남은 예산" value="1,540,000원" />
              <div className="landing-dashboard-card landing-dashboard-card-wide">
              ...
              <div className="landing-dashboard-card">
                ...
                <PreviewRow label="청년 지원 정책" value="3개" />
              </div>
              <div className="landing-dashboard-card">
                <p className="landing-panel-title">관심 금융상품</p>
                <PreviewRow label="청년 우대 적금" value="1개" />
              </div>
            </div>
          </div>
        </div>
...
function FinalCtaSection({ startPath, startLabel }) {
  return (
    <section className="landing-start" id="landing-start">
      <div className="landing-shell">
        <div className="landing-start-box">
          <h2 className="landing-start-title">
            다음 월급날까지,
            <br />
            더 계획적으로 관리해보세요.
          </h2>
          <p className="landing-start-desc">
            복잡한 계산은 더모아에 맡기고
            <br className="landing-text-break" />
            오늘 사용할 수 있는 금액부터 확인해보세요.
          </p>
          <Link to={startPath} className="landing-start-button">
            {startLabel}
          </Link>
        </div>
      </div>
...
function CssBars() {
  return (
    <div className="landing-css-bars" aria-label="카테고리 소비 비중 예시">
      <span className="landing-css-bar landing-css-bar-food" />
      <span className="landing-css-bar landing-css-bar-life" />
      <span className="landing-css-bar landing-css-bar-save" />
    </div>
  );
}
