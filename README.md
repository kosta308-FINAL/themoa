...
`main`은 배포 브랜치, `dev`는 기능 브랜치들을 먼저 모아 충돌·회귀를 확인하는 통합 브랜치입니다.

- **main** — 배포 브랜치. push되면 GitHub Actions가 변경된 경로(`frontend/**`, `backend/**`)를 감지해 자동 배포합니다.
- **dev** — 통합 브랜치. 작업 브랜치의 PR을 먼저 여기로 모아 통합한 뒤 main으로 병합합니다.
- **작업 브랜치** — `feat/*`(기능 추가), `fix/*`(버그 수정), `refactor/*`(리팩터링), `chore/*`(설정·잡무), `style/*`(스타일·UI 조정) 접두사로 작업 단위를 구분해 dev에서 분기하고, 작업이 끝나면 dev로 PR을 올립니다.
- **개인 브랜치**(`sol-dev` 등) — 팀원별로 여러 기능을 이어서 작업할 때 쓰는 개인 통합 브랜치로, dev·main과 주기적으로 동기화하면서 완료된 작업 단위마다 PR로 병합합니다.

PR이 `dev`에 병합되면 [discord-pr-merge.yml](.github/workflows/discord-pr-merge.yml)이 PR 번호·제목·본문·링크를 팀 Discord 채널로 자동 공유해, 회의 없이도 누가 무엇을 병합했는지 놓치지 않도록 했습니다.
