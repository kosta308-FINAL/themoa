...
/**
 * 카카오 콜백 이후 프론트가 호출하는 API(auth.md §6·§8). 인가 코드 교환·사용자 조회 자체는
 * spring-boot-starter-oauth2-client가 {@code /api/oauth2/authorization/kakao} →
 * {@code /api/login/oauth2/code/kakao}에서 처리하고(SecurityConfig), 여기서는 그 결과를
...
public class OAuthController {
    ...
    public ResponseEntity<ApiResponse<OAuthExchangeResponse>> exchange(@Valid @RequestBody OAuthExchangeRequest request) {
        ...
        if (result.requiresSignup()) {
            return ResponseEntity.ok(ApiResponse.success(
                    OAuthExchangeResponse.requiresSignup(result.signupTicket(), result.nickname())));
        }
    ...
    /** 카카오 신규 회원 가입 완료(이메일 인증·성별·출생일 제출). 성공 시 즉시 자동 로그인. */
    @PostMapping("/kakao/complete-signup")
    public ResponseEntity<ApiResponse<TokenResponse>> completeKakaoSignup(
            @Valid @RequestBody KakaoSignupCompleteRequest request) {
        IssuedTokens tokens = kakaoAuthService.completeSignup(request, LocalDateTime.now());
