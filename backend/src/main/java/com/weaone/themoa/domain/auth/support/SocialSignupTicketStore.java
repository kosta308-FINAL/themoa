...
/**
 * 소셜 신규 회원의 신원(provider·사용자 ID·닉네임)을, 이메일 인증·성별·출생일 제출(가입 완료) 때까지
...
public class SocialSignupTicketStore {
    ...
    public SocialSignupTicketStore(AuthProperties properties, RefreshTokenGenerator tokenGenerator) {
    ...
    public String issue(SocialProvider provider, String providerUserId, String nickname, String email) {
        String token = tokenGenerator.generate();
        tickets.put(token, new Ticket(provider, providerUserId, nickname, email));
        return token;
    }
    ...
    /** @param email 구글처럼 provider가 이미 검증한 이메일이 있을 때만 채워진다(그 외 null). */
    public record Ticket(SocialProvider provider, String providerUserId, String nickname, String email) {
    }
}
