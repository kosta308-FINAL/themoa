 ...
 *
 * @param callTimeout           EasyCodef SDK가 자체 커넥션/응답 타임아웃을 노출하지 않아, 블로킹 호출을 별도
 *                              스레드에서 실행해 이 값으로 우리가 직접 데드라인을 강제한다. 커넥션 등록(로그인)처럼
 *                              빠른 응답이 기대되는 호출용.
 * @param approvalListTimeout   승인내역 조회는 CODEF가 스크래핑 방식이라 3개월 조회 실측이 약 3분 걸린다
 *                              (cardtransaction.md §6-C). callTimeout보다 훨씬 넉넉하게 잡아야 한다.
 */
@Validated
...
public record CodefProperties(
        ...
        @NotNull Duration approvalListTimeout
) {
}
